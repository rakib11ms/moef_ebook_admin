...
const UpdateNewsAndNotice = () => {
  ...
  async function fetchData() {
    axios.get(`/api/notice/${noticeID}`).then(res => {
      if (res.data.status === 200) {
        ...
        settype(res.data.single_document.type)
        if(res.data.single_document.file !==null){
            setFileUploadCheckBox(true)
        }
      }
    }
    )
  ...
  const handleSubmit = (e) => {
    ...
    axios.post(`/api/update-single-document/${noticeID}`, formData, config).then((res) => {
      if (res.data.status === 200) {
