...
class MeetingController extends Controller
{
    ...
    public function createMeeting(Request $request)
    {
        ...
        $links = [
            'https://meet.google.com/ywp-qapp-zie',
        ...
        $group_meeting_message = $request->group_meeting_message;
        ...
        if ($group_meeting_message) {
            $new = new Message();
            $new->message = "Dear concern, 
            An online meeting will be held on " . $meeting->meeting_date . " at " . $meeting->meeting_time . ".
            Please click the link below to join in time: " . $meeting->meeting_link . "
            - ADMIN";
            $new->sender = $request->sender_name;
            $new->user_id = $request->created_by;
            $new->save();
    ...
    public function viewParticipantUsersByMeetingId($meetingId)
    {
        ...
        } else if ($participant_users->participant_users == 'সকল') {

            $users = User::where('activeStatus', 1)->get();
            return response()->json(
                [
                    'status' => 200,
                    'users' => $users,
